...
public abstract class AbstractCursor implements CrossProcessCursor, Cursor {
    ...
    public boolean update(int columnIndex, Object obj) {
        ...
        Long rowid = new Long(getLong(this.mRowIdColumnIndex));
        if (rowid == null) {
            throw new IllegalStateException("null rowid. mRowIdColumnIndex = " + this.mRowIdColumnIndex);
        }
        synchronized (this.mUpdatedRows) {
            Map<String, Object> row = (Map) this.mUpdatedRows.get(rowid);
            if (row == null) {
                row = new HashMap();
    ...
    protected boolean isFieldUpdated(int columnIndex) {
        if (this.mRowIdColumnIndex != -1 && this.mUpdatedRows.size() > 0) {
            Map<String, Object> updates = (Map) this.mUpdatedRows.get(this.mCurrentRowID);
    ...
    protected Object getUpdatedField(int columnIndex) {
        return ((Map) this.mUpdatedRows.get(this.mCurrentRowID)).get(getColumnNames()[columnIndex]);
    }
