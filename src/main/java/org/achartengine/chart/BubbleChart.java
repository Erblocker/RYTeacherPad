...
public class BubbleChart extends XYChart {
    ...
    public void drawSeries(Canvas canvas, Paint paint, List<Float> points, SimpleSeriesRenderer seriesRenderer, float yAxisValue, int seriesIndex, int startIndex) {
        ...
        for (int i = 0; i < length; i += 2) {
            ...
            Paint paint2 = paint;
            drawCircle(canvas2, paint2, ((Float) points.get(i)).floatValue(), ((Float) list.get(i + 1)).floatValue(), (float) ((series.getValue((i / 2) + startIndex) * coef) + 2.0d));
        }
    }
    ...
    protected ClickableArea[] clickableAreasForPoints(List<Float> points, List<Double> values, float yAxisValue, int seriesIndex, int startIndex) {
        ...
        for (int i = 0; i < length; i += 2) {
            ...
            ret[i / 2] = new ClickableArea(new RectF(((Float) points.get(i)).floatValue() - ((float) size), ((Float) points.get(i + 1)).floatValue() - ((float) size), ((float) size) + ((Float) points.get(i)).floatValue(), ((Float) points.get(i + 1)).floatValue() + ((float) size)), ((Double) values.get(i)).doubleValue(), ((Double) values.get(i + 1)).doubleValue());
        }
        return ret;
    }
