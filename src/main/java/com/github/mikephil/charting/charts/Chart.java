...
public abstract class Chart<T extends ChartData<? extends DataSet<? extends Entry>>> extends ViewGroup implements ChartInterface {
    ...
    public boolean valuesToHighlight() {
        if (this.mIndicesToHighlight == null || this.mIndicesToHighlight.length <= 0 || this.mIndicesToHighlight[0] == null) {
            return false;
        }
        return true;
    }
    ...
    public boolean saveToPath(String title, String pathOnSD) {
        ...
        try {
            OutputStream stream = new FileOutputStream(new StringBuilder(String.valueOf(Environment.getExternalStorageDirectory().getPath())).append(pathOnSD).append("/").append(title).append(".png").toString());
    ...
    public boolean saveToGallery(String fileName, int quality) {
        ...
        File file = new File(new StringBuilder(String.valueOf(Environment.getExternalStorageDirectory().getAbsolutePath())).append("/DCIM").toString());
        ...
        try {
            ...
            try {
                ...
                values.put(MediaColumns.DATE_ADDED, Long.valueOf(currentTime));
                values.put(MediaColumns.MIME_TYPE, "image/jpeg");
                values.put(VideoColumns.DESCRIPTION, "MPAndroidChart-Library Save");
                values.put("orientation", Integer.valueOf(0));
                values.put("_data", filePath);
                values.put(MediaColumns.SIZE, Long.valueOf(size));
                return getContext().getContentResolver().insert(Media.EXTERNAL_CONTENT_URI, values) != null;
