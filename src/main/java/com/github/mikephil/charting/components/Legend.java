...
public class Legend extends ComponentBase {
    ...
    public void calculateDimensions(Paint labelpaint, ViewPortHandler viewPortHandler) {
        if (this.mPosition == LegendPosition.RIGHT_OF_CHART || this.mPosition == LegendPosition.RIGHT_OF_CHART_CENTER || this.mPosition == LegendPosition.LEFT_OF_CHART || this.mPosition == LegendPosition.LEFT_OF_CHART_CENTER || this.mPosition == LegendPosition.PIECHART_CENTER) {
            ...
            int i2 = 0;
            while (i2 < labelCount) {
                boolean drawingForm = this.mColors[i2] != -2;
                ...
                if (this.mLabels[i2] != null) {
                    ...
                    requiredWidth = (requiredWidth + (drawingForm ? this.mFormToTextSpace + this.mFormSize : 0.0f)) + ((FSize) calculatedLabelSizes.get(i2)).width;
                } else {
            ...
            if (this.mCalculatedLineSizes.length == 0) {
                ...
                i = this.mCalculatedLineSizes.length - 1;
            }
            this.mNeededHeight = (((float) i) * labelLineSpacing) + length;
        } else {
