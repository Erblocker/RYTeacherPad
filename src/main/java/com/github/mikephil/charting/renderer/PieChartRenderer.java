            ...
            if (dataSet.isVisible()) {
                ...
                for (int j = 0; j < entries.size(); j++) {
                    ...
                    if (((double) Math.abs(((Entry) entries.get(j)).getVal())) > 1.0E-6d) {
                        float x = (float) ((((double) (r - circleRadius)) * Math.cos(Math.toRadians((double) ((angle + newangle) * this.mAnimator.getPhaseY())))) + ((double) center.x));
                        float y = (float) ((((double) (r - circleRadius)) * Math.sin(Math.toRadians((double) ((angle + newangle) * this.mAnimator.getPhaseY())))) + ((double) center.y));
                        this.mRenderPaint.setColor(dataSet.getColor(j));
