...
public class AnswerSheetV2SelectQuestionCorrectActivity extends BaseActivity {
    ...
    public boolean onOptionsItemSelected(MenuItem menuItem) {
        ...
        if (menuItem.getItemId() == 16908332) {
            ...
            LessonPrepareCorrectAnswerSheetV2Component.refreshAnsweredStudentList(new Runnable() {
                public void run() {
                    AnswerSheetV2SelectQuestionCorrectActivity.this.refresh();
                }
            }, null);
            ...
            if (this.mnType != 2) {
                ...
                this.mszCorrectAnswer.replace(szAnswer, "");
            } else {
                String szTemp = "";
                this.mszCorrectAnswer += szAnswer;
                for (int i = 0; i < 26; i++) {
                    if (this.mszCorrectAnswer.contains(String.valueOf(i + 65))) {
                        szTemp = new StringBuilder(String.valueOf(szTemp)).append(String.valueOf(i + 65)).toString();
                    }
                }
                this.mszCorrectAnswer = szTemp;
            }
            try {
    ...
    private void initTable() {
        ...
        this.mBarChart.setOnChartValueSelectedListener(new OnChartValueSelectedListener() {
            public void onValueSelected(Entry e, int dataSetIndex, Highlight h) {
                Iterator it = AnswerSheetV2SelectQuestionCorrectActivity.this.mTableRows.iterator();
                while (it.hasNext()) {
                    Utilities.setViewBackground((TableRow) it.next(), null);
                }
                ((TableRow) AnswerSheetV2SelectQuestionCorrectActivity.this.mTableRows.get(e.getXIndex())).setBackgroundResource(R.drawable.background_blueframe);
            }
    ...
    private boolean refresh() {
                        ...
                        if (jsonData.getJSONArray("category").length() > 0) {
                            ...
                            for (i = 0; i < jsonArray.length(); i++) {
                                ...
                                for (int j = 0; j < arrQuestions.length(); j++) {
                                    ...
                                    if (oneQuestion.getString("guid").equalsIgnoreCase(this.mszResourceGUID) && oneQuestion.has("answer")) {
                                        szStudentName = oneData.szRealName;
                                        szAnswer = oneQuestion.getString("answer").toUpperCase();
                                        bFound = true;
                                        nValidCount++;
                                        break;
                                    }
        ...
        for (i = 0; i < arrNames.size(); i++) {
            ...
            View tableRow = new TableRow(this.mContext);
            ...
            LayoutParams params = (TableRow.LayoutParams) title.getLayoutParams();
            ...
            title.setGravity(19);
            tableRow.addView(title);
            params = (TableRow.LayoutParams) title.getLayoutParams();
