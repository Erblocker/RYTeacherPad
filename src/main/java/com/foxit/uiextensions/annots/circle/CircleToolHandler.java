...
public class CircleToolHandler implements ToolHandler {
    ...
    public CircleToolHandler(Context context, PDFViewCtrl pdfViewCtrl, ViewGroup parent) {
    ...
    private void createAnnot() {
        if (this.mPdfViewCtrl.isPageVisible(this.mLastPageIndex)) {
            ...
            try {
                ...
                final Circle newAnnot = (Circle) page.addAnnot(6, bboxRect);
                ...
                this.mPdfViewCtrl.addTask(new EditAnnotTask(new CircleEvent(1, undoItem, newAnnot, this.mPdfViewCtrl), new Callback() {
                    public void result(Event event, boolean success) {
                        if (success) {
                            DocumentManager.getInstance(CircleToolHandler.this.mPdfViewCtrl).onAnnotAdded(page, newAnnot);
                            DocumentManager.getInstance(CircleToolHandler.this.mPdfViewCtrl).addUndoItem(undoItem);
                            if (CircleToolHandler.this.mPdfViewCtrl.isPageVisible(CircleToolHandler.this.mLastPageIndex)) {
                                RectF viewRect = null;
                                try {
                                ...
                                CircleToolHandler.this.mPdfViewCtrl.convertPdfRectToPageViewRect(viewRect, viewRect, CircleToolHandler.this.mLastPageIndex);
                                Rect rect = new Rect();
                                viewRect.roundOut(rect);
                                rect.inset(-10, -10);
                                CircleToolHandler.this.mPdfViewCtrl.refresh(CircleToolHandler.this.mLastPageIndex, rect);
                                CircleToolHandler.this.mTouchCaptured = false;
                                CircleToolHandler.this.mLastPageIndex = -1;
                                CircleToolHandler.this.mDownPoint.set(0.0f, 0.0f);
                                if (!CircleToolHandler.this.mIsContinuousCreate) {
                                    ((UIExtensionsManager) CircleToolHandler.this.mPdfViewCtrl.getUIExtensionsManager()).setCurrentToolHandler(null);
                                    CircleToolHandler.this.mToolBtn.setSelected(false);
                                }
                            }
                        }
                    }
    ...
    public void init() {
        ...
        if (this.mColor == 0) {
            this.mColor = PropertyBar.PB_COLORS_CIRCLE[0];
        }
        if (this.mOpacity == 0) {
            this.mOpacity = 100;
        }
        ...
        this.mToolBtn.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                CircleToolHandler.this.changeStatusForClick();
            }
        });
        this.mToolBtn.setOnLongClickListener(new OnLongClickListener() {
            public boolean onLongClick(View v) {
                if (DocumentManager.getInstance(CircleToolHandler.this.mPdfViewCtrl).canAddAnnot() && ((UIExtensionsManager) CircleToolHandler.this.mPdfViewCtrl.getUIExtensionsManager()).getCurrentToolHandler() != CircleToolHandler.this) {
                    ((UIExtensionsManager) CircleToolHandler.this.mPdfViewCtrl.getUIExtensionsManager()).setCurrentToolHandler(CircleToolHandler.this);
                }
                return true;
            }
        });
